function countBy(items, getGroupName) {
    let counts = [];

    for(let item of items) {
        let name = getGroupName(item);
...
function dominantDirection(string) {
    return countBy(string, c => getDirection(c.codePointAt(0))).reduce((a, b) => a > b ? a : b);
}

console.log(dominantDirection('ބޖޛޚބޑޖލޞ英国的狗说woof俄罗斯的狗说тяв'));
